...
export function BookingFlow() {
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden">
      ...
      <div className="relative z-10 mx-auto grid max-w-[1312px] gap-3 px-2 pb-6 pt-[76px] md:gap-4 md:px-16 md:pt-[120px] lg:grid-cols-[1fr_440px]">
        <div className="glass rounded-4xl p-5 shadow-[0_30px_80px_rgba(0,0,0,0.45)] md:rounded-5xl md:p-10">
          {step === 2 ? (
            <ContactForm
              initial={contact}
              onBack={() => setStep(1)}
        ...
        <aside className="glass rounded-4xl p-5 md:rounded-5xl md:p-8">
...
function StepNav({ step, onBack }: { step: 1 | 2 | 3; onBack?: () => void }) {
  ...
  return (
    <nav className="glass fixed inset-x-2 top-2 z-30 flex h-[52px] items-center justify-between rounded-3xl pl-4 pr-1.5 md:inset-x-16 md:top-6 md:h-16 md:rounded-full md:pl-7">
      <Logo />
      <ol className="flex gap-0.5 rounded-2xl bg-white/[0.06] p-1">
        {steps.map((s, i) => {
          ...
          return (
            <li key={s}>
              {done && n === 1 && onBack ? (
                <button type="button" onClick={onBack} className="flex h-9 items-center rounded-xl px-3 text-[13px] font-medium text-label hover:text-white md:px-4">
                  ✓ <span className="ml-1.5 hidden sm:inline">{s}</span>
                </button>
              ) : (
                <span
                  aria-current={current ? "step" : undefined}
                  className={`flex h-9 items-center rounded-xl px-2.5 text-[13px] md:px-4 ${
