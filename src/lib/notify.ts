...
import { sendEmail, sendSms } from "./brevo";
import { driverContact } from "./driver";
...
export async function notifyBookingPaid(b: Booking) {
  ...
  const { email: driverEmail, phone: driverPhone } = await driverContact();
  if (driverEmail) {
    await sendEmail(
      driverEmail,
...
export async function notifyDriverOnTheWay(b: Booking) {
  await sendSms(b.phone, `Eco Driver : votre chauffeur est en route pour la course ${b.reference}.`);
}

export async function notifyDriverArrived(b: Booking) {
  await sendSms(b.phone, `Eco Driver : votre chauffeur est arrivé au point de prise en charge.`);
}

/** Lien de réinitialisation du mot de passe, valable une heure. */
export async function notifyPasswordReset(to: string, url: string) {
  await sendEmail(
    to,
    "Réinitialiser votre mot de passe Eco Driver",
    layout(
      "Réinitialiser votre mot de passe",
      `<p style="color:#C7C7CC;line-height:1.6">Ce lien est valable une heure et ne fonctionne qu'une fois. Si vous n'êtes pas à l'origine de cette demande, ignorez ce message : votre mot de passe reste inchangé.</p>${button(url, "Choisir un nouveau mot de passe")}`
    )
  );
}
